...
module.exports = async function contentfulFetch({
  ...
  const contentfulClientOptions = {
    ...
    responseLogger: response => {
      const meta = [`size: ${response.headers[`content-length`]}B`, `response id: ${response.headers[`x-contentful-request-id`]}`, `cache: ${response.headers[`x-cache`]}`];
      reporter.verbose(`${response.config.method} /${response.config.url}: ${response.status} ${response.statusText} (${meta.join(` `)})`);
    }
  };
  ...
  try {
    ...
    if (locales.length === 0) {
      reporter.panic({
        ...
        context: {
          sourceMessage: `Please check if your localeFilter is configured properly. Locales '${contentfulLocales.map(item => item.code).join(`,`)}' were found but were filtered down to none.`
        }
      });
    }
